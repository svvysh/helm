...
func readmeTemplate() string {
	lines := []string{
		...
		"",
		"This `specs/` directory (tracked in this repo under `docs/specs/`) contains the specification bundles and templates used to build and evolve the Cross-Project Spec Runner CLI itself.",
		"",
		"The workflow is:",
		"",
		...
		"## Folder layout",
		"",
		"- `implement.prompt-template.md` — worker prompt template (strict/parallel aware).",
		...
		"- Global settings: `~/.helm/settings.json` — default models, reasoning effort, and acceptance commands.",
		"- `spec-00-example/` — sample spec showing the expected structure.",
		"",
		"Example spec folders:",
		"",
		"- `spec-00-example/` — sample spec illustrating structure and metadata.",
		"",
		...
		"   - `node specs/implement-spec.mjs specs/spec-00-example`",
...
func specSplittingGuideTemplate() string {
	lines := []string{
		"# Spec Splitting Guide",
		"",
		...
		"5. Update the dependency graph (see \"sample-dependency-graph.json\" if generated) so the TUI can surface blocking relationships.",
		"",
...
func buildImplementPrompt(mode config.Mode) string {
	...
	template := strings.Join([]string{
		...
		"- Specs root: `specs`",
		"- Each spec lives inside its own `spec-XX-*` folder next to this template.",
		"",
		"You may:",
